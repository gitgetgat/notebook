...
import { NAME_LABELS, DIR_LABELS } from "./name-sort-lang";
...
function getNavItems(ele) {
  ...
  if (ele.child && ele.child.length) {
    ...
    const nameLabels = NAME_LABELS.filter((i) =>
      i.parentDir.includes(ele.name)
    );
    const files = fs
      .readdirSync(ele.link, { withFileTypes: true })
      .sort((a, b) => {
        return (
          (nameLabels.find((i) => i.en === upath.trimExt(a.name)) || {})
            .sortKey -
          (nameLabels.find((i) => i.en === upath.trimExt(b.name)) || {}).sortKey
        );
      });

    if (files.length > 0) {
      if (
        files.filter((file) => upath.trimExt(file.name) === "index").length > 0
      ) {
